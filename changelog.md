...
- 🟥 breaking changes to the fed api (api between the popup and your app)
  ...
  - i also changed the names of the tokens that get returned
- 🟥 reworked tokens, now we have `LoginKeys`, `LoginProof`, and `LoginClaim` tokens
- 🟥 moved the passport's `name` from LoginKeys to LoginProof
- 🔶 tweaked authfile format, but its versioned and so should not cause breakage
- 🍏 add `Token` utility for making quick simple tokens
